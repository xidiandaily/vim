...
function CSTAG_ueproj()
    ...
    if(executable('ctags'))
        if(g:iswindows==1)
            ...
            if !filereadable('ueprojc.tags') && !filereadable('pgamelua.tags')
                let choice = 1
            else
            ...
            if choice == 1 || choice == 2
                silent! execute "!ctags.exe -f ueprojc.tags  -D \"UFUNCTION(x)=;\" -D \"UPROPERTY(x)=;\" --languages=C,+C++,+CMake,+Automake,+Autoconf --c++-kinds=+p --fields=+iaS --extras=+q -R ."
                silent execute ':!'.getenv('VIMPROJ').'/mytag_helper/mytag_helper.exe -r -t '.getcwd().'/ueprojc.tags'
            endif

            "if choice == 1 || choice == 3
            "    call CTags_ueproj_lua()
            "endif

            "if choice == 1 || choice == 4
            "    call CTags_ueproj_xml()
            "endif

            call ctrlp#mycmd#UEProjTag()
            "set tags+=pgamelua.ctrlptags,pgamexml.ctrlptags,ueprojc.ctrlptags
            set tags+=ueprojc.ctrlptags
        else
...
function ASTYLE_ueproj()
    let g:astyle_file=expand("%")
    silent execute "!astyle -A1M40Sjk1n --mode=c ".getcwd()."/".g:astyle_file
endfunction


function UpdatePath_ueproj(root)
    set path += "LyraStarterGame/Source/LyraGame"
endfunction
...
function! InitUEProj()
    ...
    :set path+=$VIMPROJ/vimlib/cppstl/cpp_src,
    let g:chiylown_func_dict["CSTAG"]["ueproj"]="CSTAG_ueproj"
    let g:chiylown_func_dict["ASTYLE"]["ueproj"]="ASTYLE_ueproj"
endfunction
